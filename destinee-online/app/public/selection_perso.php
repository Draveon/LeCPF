...
if ($connexion_compte == 1)
{
	...
	if (!isset($_POST['action'])) {
		...
		if ($_POST['action'] == "creer1") {
			...
			$i = 0;
			$tabRaces = [];
			$req = mysql_query("SELECT race, att, def, ini, cha, end, pui, aptitude, t_favoris, image FROM races WHERE race != 'Primordial' ORDER BY race ASC") or die('Erreur SQL !<br>'.mysql_error());
