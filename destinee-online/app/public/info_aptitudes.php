...
$reqRace = mysql_query('SELECT `aptitude` FROM races WHERE race = \''.$race.'\'') or die(mysql_error());
$datRace = mysql_fetch_row($reqRace);
$reqClasse = mysql_query("SELECT `aptitude` FROM classes WHERE classe = '".$classe."'") or die('Erreur SQL !<br>'.mysql_error());
$datClasse = mysql_fetch_row($reqClasse);

// Si l'aptitude de la race est égale à celle de la classe, le maximum est de 140
if ($datRace[0] == $datClasse[0]) {
		$max += 40;
} else {
