...
if ($q > 0)					// si plus d'un jour s'est passé depuis la dernière mise à jour des créatures
{
	...
	for ($i=6; isset($batiment_id[$i]); $i++) // les id jusqu'à 5 sont des tours de magie
	{
		if ($batiment_id[$i] != '0')	// bâtiment construit, on traite
		{
			...
			$nombrepJ = $da2[0];
			$nivo_bat = $da2[1];
			// On set le nouveau nombre de créature... avec un max à 10 demi-journées de stack. (Jumelé avec la ligne 235 de batiments.php)
			$nbr = round( min( 10 *     ($nombrepJ * $crea_obj * ($crea_polit + $crea_palat) * $crea_cha),
