...
/*
	...
	// Ajout du héros à sa faction à chaque chargement de la page de statut, ça fait chier mais c'est pour être sûr
	addHeroToFaction();
	getRangJoueur($nom);
	// MAJ faction

	$factionId = getFactionIdByMember($nom);
	$nivFaction = getFactionNivByMember($nom);
	$palierVic = getFactionPalierByMember($nom);
	$vicAtt = vicFacAtt($factionId);
	$vicDef = vicFacDef($factionId);
	$totvic = $vicAtt+$vicDef;
	$nbMembersFac = nbMembers($factionId)/15;
	$vic = round( ($totvic / $nbMembersFac), 1 );

	// if ($nom == 'Draveon') {
		// $reqFactions = mysql_query("SELECT DISTINCT factions.id, factions.nom, factions.niv, factions.prochpalier, factions.maj_new_niv FROM factions INNER JOIN factions_membres ON factions_membres.faction_id = factions.id WHERE factions_membres.faction_id = ".$factionId." ");
		// $row = mysql_fetch_assoc($reqFactions);
		// var_dump($row['id']);
		...
		if ($vic >= $palierVic) {
			...
			// $isUpdated = checkIfFactionIsUpdated($factionId);
			// if ($isUpdated == 0) {
				// var_dump($isUpdated);
				 // echo "la faction peut-être augmentée et a atteint son palier";
				executeMajFaction($nom, $factionId, $palierVic, $nivFaction);
