...
if ($dateAncienne < $dateActuelle) {
 ...
 $sql = "UPDATE `joueurs_connexions` SET `connexion_av_dernière` = '".$dateActuelle."' WHERE id = '".$id."' ";
 $result = mysql_query($sql);
 if (!$result) {
   echo "Impossible d'exécuter la requête ($sql) dans la base : " . mysql_error();
 }
...
mysql_query("UPDATE `joueurs_connexions` SET `connexion_dernière` = '".$temps."' WHERE `id` = '".$id."'") or die('Erreur SQL 137 !<br>'.mysql_error());
