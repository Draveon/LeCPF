...
if (!isset($_POST['qtee']))
{
	...
	// Modificateur au prix dépendant du charisme
...
';
	...
	<form method="post" action="recruter_detruit.php">	
	<table class="discr100"><tbody>
	<tr>	<td rowspan="13" class="discr100img"><img src="'.$image.'" width="100px" height="100px" alt="'.$creature.'" border="1"></td><td colspan="2">&nbsp;</td></tr>
...
/*	<tr>	<td colspan="2">Expulsion de toutes les créatures : </td></tr>
	<tr>	<td colspan="2"><div id="idexpulser"></div></td></tr>
	<tr style="if"><td colspan="2" style="text-align:center;padding:10px;">
		<input type="submit" name="ExpulsionBtn" value="Expulser >>">
	</td></tr>*/
		echo '	</tbody></table>';
	...
	<div id="creatDetails" style="display:none;">
		...
		<div style="width:95%; margin:0px auto;">
			...
			<div class="tableRecrL">
				...
				<div><span class="tdRecr"></span></div>
				<div><span class="tdRecr"></span></div>
				<div><span class="tdRecr"></span></div>
			</div>';
	echo '
...
else
{
	...
	if ((isset($_POST['recrutement']))&&(is_numeric($_POST['recrutement']))) {
		...
		} else if (isset($_POST['ExpulsionBtn'])) {
			$action_expulser = true;
		}
	...
	} else if ($action_expulser) {
		$sql = "UPDATE `joueurs_créatures_id` SET `".$id."` = 0 WHERE nom = '".$nom."'";
		$req = mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());
	}
