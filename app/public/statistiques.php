...
if ($serveur_off != 0)
{
	...
	$req = mysql_query("SELECT COUNT(DISTINCT id_compte) AS nb FROM joueurs_heros INNER JOIN joueurs_connexions ON joueurs_connexions.nom = joueurs_heros.nom WHERE `connexion_dernière` > 'connexion_av_dernière'") or die('Erreur SQL !<br>'.mysql_error());
	$dat = mysql_fetch_row($req);
	$actif = $dat[0];
	$req = mysql_query("SELECT COUNT(DISTINCT id_compte) AS nb FROM joueurs_heros INNER JOIN joueurs_connexions ON joueurs_connexions.nom = joueurs_heros.nom WHERE `connexion_dernière` > '$limite'") or die('Erreur SQL !<br>'.mysql_error());
	$dat = mysql_fetch_row($req);
	$a24h = $dat[0];
	$req = mysql_query("SELECT COUNT(DISTINCT id_compte) AS nb FROM joueurs_heros") or die('Erreur SQL !<br>'.mysql_error());
	$dat = mysql_fetch_row($req);
	$all = $dat[0];
