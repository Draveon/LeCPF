...
// Gain or
$gain_or_min = round($gainparmin * $or_obj * ($or_politique + $or_palatin) * $temps_ecoulement * $bonus_or_niveau  * (2/3 + 1/3 * $coef_armee_faible));
// Taxes
$taxe_or_min = round($gain_or_min * $taxe);
// Gain par les esclaves
$pop2_rentabilite = 5/8000;
$gain_esclave_min = round(1.5 * $pop2_rentabilite * $pop2 * $or_obj * ($or_politique + $or_palatin) * $temps_ecoulement * $bonus_or_niveau  * (2/3 + 1/3 * $coef_armee_faible_prisio));
// Taxe esclave
...
$gain_mana_min = round(($niv_tour * 27) * $mana_obj * ($mana_politique + $mana_palatin) * $temps_ecoulement);
...
if (isset($_GET['offensive']) OR isset($_GET['defensive']) OR isset($_GET['incantemin']))
{
	...
	echo '	<td colspan="3" class="tdcen"><b>Vos stratégies ont été modifiées avec succès !</b><br>&nbsp;'."\n";
}
// rapports de combat
...
// AFFICHAGE
echo '
<div class="col-12 plank_background"><h2>Stratégie</h2></div>
<div class="col-12 plank_background">
  <div class="scroll_background str">
...
<br><input type="submit" value="Régler >>">';

// rapport d'autoespionnage
echo '<td colspan="4" class="tdcen"><br><a href="strategie.php?mode=autoespionnage">Générer un rapport sur votre compte !</a><!--<br />';
