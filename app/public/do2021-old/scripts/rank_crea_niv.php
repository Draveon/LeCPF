...
$resultat_final_pourc = array();

$sql_total = "SELECT * FROM creatures WHERE niveau = $niv_test AND id > 5 AND speciale = 0 AND constructible = 1";
$req_total = mysql_query($sql_total) or die('Erreur SQL !<br>'.mysql_error());

while ($crea = mysql_fetch_assoc($req_total)) {
	...
	$resultat_final_pourc[$nom_crea] = 0;
	...
	while ($adv = mysql_fetch_assoc($req)) {
		...
		if (!$pb) {
			...
			$vainqueur = $v['nom'];
			$vaincu = $x['nom'];
			$pourc_restant = $v['pvs_restant'] / $v['pvs_ini'] * 100;
			if ($v['id'] == $id_crea) {
				$nb_vict_crea++;
				$resultat_final_pourc[$nom_crea] += $pourc_restant;
			}
		}
	}
	$resultat_final[$nom_crea] = round($nb_vict_crea / $nb_combats * 100, 1);
	$resultat_final_pourc[$nom_crea] /= $nb_combats;
	$creas_ini[$nom_crea] = $crea['ini'];
}

asort($resultat_final);
foreach($resultat_final as $crea => $victoires) {
	$pourc_rest = round($resultat_final_pourc[$crea], 1);
	$ini = $creas_ini[$crea];
	echo "$crea ($ini ini) => $victoires% wins avec $pourc_rest% pvs restants<br>";
}
