...
function join_armada_menu ($joueur)
{
	$req = mysqli_query($db, "SELECT race, niveau FROM joueurs_heros INNER JOIN joueurs_stats on joueurs_stats.nom = joueurs_heros.nom WHERE joueurs_heros.nom = '".$joueur."'") or die(mysqli_error($db));
...
function join_province ($joueur, $province)
{
	...
	if ($data = mysqli_fetch_assoc($req))
	{
		...
		$req = mysqli_query($db, $sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysqli_error($db));
		$conso_mana = mysqli_fetch_assoc($req);
		$conso_mana = 1 - $conso_mana['magie'];
		mysqli_query($db, "UPDATE `joueurs_bonus` SET `politique` = '".$data['politique1']."', `province` = '".$province."' WHERE `nom` = '".$joueur."'") or die('Erreur SQL !<br>'.mysqli_error($db));
		mysqli_query($db, "UPDATE `joueurs_modificateurs` SET `mana_consommation` = `mana_consommation` + '".$conso_mana."' WHERE `nom` = '".$joueur."'") or die('Erreur SQL !<br>'.mysqli_error($db));
		return '';
	}
